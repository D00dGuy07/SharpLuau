...
namespace SharpLuau.Compilation.Syntax
{
	...
	public class LuauContext
	{
		...
		private Dictionary<string, string> m_TypeToOutputFile;
		private Dictionary<string, RobloxObjectPath> m_OutputFileToRobloxPath;

		public LuauContext(Dictionary<string, LuauIdentifier> dynamicSymbols, 
			Dictionary<string, string> typeToOutputFile,
			Dictionary<string, RobloxObjectPath> outputFileToRobloxPath
		)
		{
			m_DynamicSymbols = dynamicSymbols;
			m_TypeToOutputFile = typeToOutputFile;
			m_OutputFileToRobloxPath = outputFileToRobloxPath;
		}

		public LuauContext()
			: this(new(), new(), new()) {}

		public LuauContext(LuauContext original)
			: this(new(original.m_DynamicSymbols), new(original.m_TypeToOutputFile), new(original.m_OutputFileToRobloxPath)) {}
		...
		public static LuauContext Combine(LuauContext[] contexts)
		{
			...
			foreach (LuauContext context in contexts)
			{
				...
				context.m_TypeToOutputFile.ToList().ForEach(x => typeToOutputFile.Add(x.Key, x.Value));
				context.m_OutputFileToRobloxPath.ToList().ForEach(x => outputFileToRobloxPath.Add(x.Key, x.Value));
			}

			return new LuauContext(dynamicSymbols, typeToOutputFile, outputFileToRobloxPath);
		}
		...
		public Dictionary<string, string> GetTypeToOutputFileDictionary() => new(m_TypeToOutputFile);

		// Dynamic Identifier Functions

		public void AssignSymbolName(ISymbol symbol, LuauIdentifier newIdentifier) => m_DynamicSymbols[SymbolHash(symbol)] = newIdentifier;
		public void AssignSymbolName(string symbolHash, LuauIdentifier newIdentifier) => m_DynamicSymbols[symbolHash] = newIdentifier;
		public LuauIdentifier GetSymbolName(ISymbol symbol) => m_DynamicSymbols[SymbolHash(symbol)];
		public LuauIdentifier GetSymbolName(string symbolHash) => m_DynamicSymbols[symbolHash];
		...
		// Type to Output File Functions

		public void AssignTypeToFile(INamedTypeSymbol symbol, FileInfo outputFile) => m_TypeToOutputFile.Add(SymbolHash(symbol), outputFile.FullName);
		public FileInfo? GetTypeOutputFileOrDefault(INamedTypeSymbol symbol)
		{
			string? path = m_TypeToOutputFile.GetValueOrDefault(SymbolHash(symbol));
			return path != null ? new(path) : null;
		}
		public FileInfo? GetTypeOutputFileOrDefault(string symbolHash)
		{
			string? path = m_TypeToOutputFile.GetValueOrDefault(symbolHash);
			return path != null ? new(path) : null;
		}

		// Output File to Roblox Path

		public void AssignRobloxPathToFile(FileInfo outputFile, RobloxObjectPath robloxPath) => m_OutputFileToRobloxPath.Add(outputFile.FullName, robloxPath);
		public RobloxObjectPath GetRobloxPathFromFileOrDefault(FileInfo outputFile) => m_OutputFileToRobloxPath.GetValueOrDefault(outputFile.FullName);
		...
		public void WriteBinary(BinaryWriter writer)
		{
			...
			writer.Write(m_TypeToOutputFile.Count);
			foreach (var pair in m_TypeToOutputFile)
			{
				writer.Write(pair.Key);
				writer.Write(pair.Value);
			}

			writer.Write(m_OutputFileToRobloxPath.Count);
			foreach (var pair in m_OutputFileToRobloxPath)
			{
				writer.Write(pair.Key);

				writer.Write(pair.Value.Service);
				writer.Write(pair.Value.Path.Length);
				foreach (string objectName in pair.Value.Path)
	...
	public sealed class LuauImportStatement : LuauStatement
	{
		[LuauTreeProperty]
		...
		public LuauImportStatement(List<INamedTypeSymbol> typeSymbols)
		{
			Symbols = new();
			foreach (INamedTypeSymbol symbol in typeSymbols)
				Symbols.Add(LuauContext.SymbolHash(symbol));
		}

		public LuauImportStatement(List<string> symbolHashes)
		{
		...
		public override string WriteText(LuauContext context)
		{
			Dictionary<string, int> variableNameOverloads = new();
			Dictionary<string, RobloxObjectPath> filesToImport = new();
			foreach (string symbol in Symbols)
			{
				FileInfo? file = context.GetTypeOutputFileOrDefault(symbol);
				if (file == null)
					throw new Exception("Missing file association");
				string filePath = file.FullName;

				if (!filesToImport.ContainsKey(filePath))
				{
					RobloxObjectPath objectPath = context.GetRobloxPathFromFileOrDefault(file);
					filesToImport[filePath] = objectPath;

					string vName = objectPath.Path.Last();
					if (variableNameOverloads.ContainsKey(vName))
						variableNameOverloads[vName]++;
					else
						variableNameOverloads[vName] = 0;
				}

				string variableName = filesToImport[filePath].Path.Last();
				int overloadCount = variableNameOverloads[variableName];

				context.AssignSymbolName(symbol, $"{(overloadCount == 0 ? variableName : $"{variableName}{overloadCount}")}.{context.GetSymbolName(symbol).Text}");
			}

			// Reset overload counters because they needs to be calculated again as the import statements are generated
			variableNameOverloads = new();

			StringBuilder output = new();
			foreach(var pair in filesToImport)
			{
				string variableName = pair.Value.Path.Last();
				if (variableNameOverloads.ContainsKey(variableName))
					variableNameOverloads[variableName]++;
				else
					variableNameOverloads[variableName] = 0;

				int overloadCount = variableNameOverloads[variableName];


				output.Append($"local {(overloadCount == 0 ? variableName : $"{variableName}{overloadCount}")} = {pair.Value.GetRequireLine()}\n");
			}
	...
	public sealed class LuauMemberAccess : LuauExpression
	{
		...
		public LuauMemberAccess(LuauExpression expression, LuauIdentifier member, bool isMemberMethod = false)
		{
		...
		public override string WriteText(LuauContext context) => 
			string.Format("{0}{1}{2}", Expression.WriteText(context), IsMemberMethod ? ':' : ".", Member.WriteText(context));
	...
	public sealed class LuauDynamicIdentifier : LuauIdentifier
	{
		...
		public LuauDynamicIdentifier(string symbolHash)
			: base(symbolHash) { }
	...
	public sealed class LuauTableItemExpression : LuauExpression
	{
		[LuauTreeProperty]
		public override ExpressionKind Kind => ExpressionKind.TableItem;

		[LuauTreeChild]
		public LuauExpression Value;

		[LuauTreeChild]
		public LuauExpression? Key;

		public LuauTableItemExpression(LuauExpression value, LuauExpression? key = null)
		{
			Value = value;
			Key = key;
		}
		...
		public override string WriteText(LuauContext context)
		{
			// I honestly can't tell if this is readable or not
			return Key != null ? 
				($"{( Key is LuauIdentifier ? Key.WriteText(context) : $"[{Key.WriteText(context)}]" )} = {Value.WriteText(context)}") : 
				(Value.WriteText(context));
		}
	...
	public sealed class LuauTableExpression : LuauExpression
	{
		...
		[LuauTreeChildren(false)]
		public List<LuauTableItemExpression> Contents;
		...
		public LuauTableExpression(List<LuauTableItemExpression> contents, bool shouldSpread = false)
		{
			Contents = contents;
		...
		public override string WriteText(LuauContext context)
		{
			...
			stringBuilder.Append(ShouldSpread ? "{\n\t" : '{');

			// Fill all of the contents
			int index = 0;
			int lastIndex = Contents.Count - 1;
			foreach (LuauTableItemExpression expression in Contents)
			{
